...
<body>
	<div class="tab-switcher">
		<button onclick="switchTab(event, 'tab-add')" class="active">Добавление</button>
		<button onclick="switchTab(event, 'tab-edit')">Редактирование</button>
		<button onclick="switchTab(event, 'tab-publish')">Публикация</button>
		<button onclick="switchTab(event, 'tab-users')">Пользователи</button>
	</div>

	<div class="tab active" id="tab-add">
		<!-- Контент для вкладки "Добавление" -->
		<div class="content">
	...
	<div class="tab" id="tab-edit">
		<!-- Контент для вкладки "Редактирование" -->
		<h2>Редактирование</h2>
		<p>Здесь будет форма для редактирования существующих элементов.</p>
	</div>

	<div class="tab" id="tab-publish">
		<!-- Контент для вкладки "Публикация" -->
		<h2>Публикация</h2>
		<p>Здесь будет форма для публикации элементов.</p>
	</div>

	<div class="tab" id="tab-users">
		<!-- Контент для вкладки "Пользователи" -->
		<h2>Пользователи</h2>
		<p>Здесь будет список пользователей и возможность их управления.</p>
	</div>

	<script>
	var userRole = "admin"; // задаем значение "admin" для переменной userRole
	function switchTab(event, tabName) {
  // Проверяем права доступа пользователя
  if (userRole === "admin") {
    // Доступ разрешен, продолжаем
    var tabs = document.getElementsByClassName("tab");

    // Проходимся по ним и скрываем все, кроме выбранной
    ...
    // Удаляем класс active у всех кнопок
		var buttons = document.getElementsByClassName("tab-switcher")[0].getElementsByTagName("button");
    ...
    // Доступ запрещен, выводим сообщение об ошибке
    alert("У вас нет прав для просмотра этой вкладки");
  }
}
